...
function getTrackerExePath(): string {
  const possiblePaths = [
    join(app.getAppPath(), 'resources', 'riot_tracker.exe'),
    join(__dirname, '../../resources/riot_tracker.exe'),
    join(process.resourcesPath || '', 'resources', 'riot_tracker.exe'),
    join(process.resourcesPath || '', 'riot_tracker.exe'),
    'd:\\Projects\\Umbral\\resources\\riot_tracker.exe'
  ]
